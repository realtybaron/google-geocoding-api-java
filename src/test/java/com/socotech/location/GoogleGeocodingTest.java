...
public class GoogleGeocodingTest {
    private GoogleGeocoding geocoder;

    @Before
    public void onSetup() {
        this.geocoder = new GoogleGeocoding.Builder().apiKey("AIzaSyCgk-CIEFxwdBbWxM3VVu0EDwBIEq4ZjpE").build();
    }

    @Test
    public void testAltLocality() throws Exception {
        Assert.assertEquals("Address is wrong", "2901 Dolan Road, Drummonds, TN 38023, United States", geocoder.findByAddress("2901 dolan road drummonds tn").address().toString());
        Assert.assertEquals("Address is wrong", "6839 Minuteman Trail, Derby, NY 14047, United States", geocoder.findByAddress("6839 minuteman derby ny").address().toString());
        Assert.assertEquals("Address is wrong", "1672 Digger Tree Court, Cool, CA 95614, United States", geocoder.findByAddress("1672 digger tree ct cool ca").address().toString());
    }

    @Test
    public void testPreciseAddress() throws Exception {
        GeocodingResponse response = geocoder.findByAddress("1316 Kenwood Avenue Austin, TX 78704");
    ...
    public void testImpreciseAddress() throws Exception {
        Geometry geometry = geocoder.findByAddress("Kenwood Avenue Austin, TX 78704").geometry();
    ...
    public void testReverseGeocoding() throws Exception {
        ...
        Address address = geocoder.findByGeometry(lat, lng).address();
